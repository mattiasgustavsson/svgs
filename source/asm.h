...
char* asm_disassemble( void* input, size_t input_size );
...
void outbuf_append( outbuf_t* outbuf, void const* data, size_t size ) {
...
void outbuf_append_str( outbuf_t* outbuf, char const* str ) {
    outbuf_append( outbuf, str, strlen( str ) );
}
...
char* asm_disassemble( void* input, size_t input_size ) {
    ...
    uint32_t* start = (uint32_t*) input ;
    uint32_t* data = start;
    uint32_t* end = start + input_size / sizeof( uint32_t );

    // Do a first pass where we just collect labels
    int labels_count = 0;
    int labels_capacity = 256;
    int* labels = (int*) malloc( sizeof( int ) * labels_capacity );
    ...
    while( data < end ) {
        ...
        if( operand_a != CPU_ARGTYPE_NONE) {
            ...
            // Process if label reference
            if( operand_a == CPU_ARGTYPE_AT_LABEL || operand_a == CPU_ARGTYPE_LABEL ) {
                // Calculate absolute offset
                int offset = (int)( ( ( (uintptr_t) data ) - (uintptr_t) start ) + operand );
                
                // Check if this label already have been added
                bool found = false;
                ...
                // Add label if it didn't already exist
                if( !found ) {
                    if( labels_count >= labels_capacity ) {
                        labels_capacity *= 2;
                        labels = (int*) realloc( labels, sizeof( int ) * labels_capacity );
                    }
                    labels[ labels_count++ ] = offset;
                }
            }
        }
        ...
        if( operand_b != CPU_ARGTYPE_NONE ) {
            ...
            // Process if label reference
            if( operand_b == CPU_ARGTYPE_AT_LABEL || operand_b == CPU_ARGTYPE_LABEL ) {
                // Calculate absolute offset
                int offset = (int)( ( ( (uintptr_t) data ) - (uintptr_t) start ) + operand );
    ...
    // Sort the labels so they appear in a logical order
    for( int i = 0; i < labels_count; ++i ) {
        for( int j = i + 1; j < labels_count; ++j ) {
            int a = labels[ i ];
            int b = labels[ j ];
            if( b < a ) {
                labels[ i ] = b;
    ...
    while( data < end ) {
        ...
        for( int i = 0; i < labels_count; ++i ) {
            ...
            if( offset == labels[ i ] ) {
                ...
                char buf[ 256 ];
                sprintf( buf, "label%d:\n", i );
                outbuf_append( &output, buf, strlen( buf ) );
            }
        }
        ...
        outbuf_append_str( &output, "\t" );
        char const* mnemonic = cpu_opinfo_mnemonic( op );
        outbuf_append_str( &output, mnemonic );
        if( optype == CPU_OPTYPE_BYTE ) {
            outbuf_append_str( &output, ".b" );
        } else if( optype == CPU_OPTYPE_WORD ) {
            outbuf_append_str( &output, ".w" );
        } else if( optype == CPU_OPTYPE_LONG ) {
            outbuf_append_str( &output, ".l" );
        } else if( optype == CPU_OPTYPE_FLOAT ) {
            outbuf_append_str( &output, ".f" );
        }


        // First operand
        if( operand_a != CPU_ARGTYPE_NONE ) {
            // Operation and operands are separated by a space
            outbuf_append_str( &output, " " );

            // Read operand value
            uint32_t operand = *data++;
    
            // Process operand value depending on its type
            switch( operand_a ) {
                case CPU_ARGTYPE_AT_LABEL:
                    outbuf_append_str( &output, "@" );
                    // Intentional fall-through to CPU_ARGTYPE_LABEL 
                case CPU_ARGTYPE_LABEL: {
                    // Calculate absolute offset
                    int offset = (int)( ( ( (uintptr_t) data ) - (uintptr_t) start ) + operand );
                ...
                case CPU_ARGTYPE_AT_AREG_DEC:
                    outbuf_append_str( &output, "@" );
                    // Intentional fall-through to CPU_ARGTYPE_AREG
                case CPU_ARGTYPE_AREG: {
                    // Write the register name
                    char buf[ 256 ];
                    sprintf( buf, "a%d%s", operand - CPU_REG_A0, operand_a == CPU_ARGTYPE_AT_AREG_INC ? "+" : operand_a == CPU_ARGTYPE_AT_AREG_DEC ? "-" : "" ); 
                    outbuf_append_str( &output, buf );
                } break;
                ...
                case CPU_ARGTYPE_DREG: {
                    // Write the register name
                    char buf[ 256 ];
                    sprintf( buf, "d%d", operand - CPU_REG_D0 ); 
                    outbuf_append_str( &output, buf );
                } break;
                ...
                case CPU_ARGTYPE_FREG: {
                    // Write the register name
                    char buf[ 256 ];
                    sprintf( buf, "f%d", operand - CPU_REG_F0 ); 
                    outbuf_append_str( &output, buf );
                } break;

                case CPU_ARGTYPE_AT_INTEGER:
                    outbuf_append_str( &output, "@" );
                    // Intentional fall-through to CPU_ARGTYPE_INTEGER
                case CPU_ARGTYPE_INTEGER: {
                    // Write the value
                    char buf[ 256 ];
                    int ival = (int) operand;
                    if( ival > -10 && ival < 10 ) {
                        sprintf( buf, "%d", ival );
                    } else {
                ...
                case CPU_ARGTYPE_AT_STACK:
                    outbuf_append_str( &output, "@" );
                    // Intentional fall-through to CPU_ARGTYPE_STACK
                case CPU_ARGTYPE_STACK: {
                    char buf[ 256 ];
                    sprintf( buf, "sp(%d)", operand );
                    outbuf_append_str( &output, buf );
                } break;
            }
        }
        ...
        if( operand_b != CPU_ARGTYPE_NONE ) {
            ...
            switch( operand_b ) {
                ...
                case CPU_ARGTYPE_AREG: {
                    // Write the register name
                    char buf[ 256 ];
                    sprintf( buf, "a%d%s", operand - CPU_REG_A0, operand_b == CPU_ARGTYPE_AT_AREG_INC ? "+" : operand_b == CPU_ARGTYPE_AT_AREG_DEC ? "-" : "" ); 
                    outbuf_append_str( &output, buf );
                } break;
